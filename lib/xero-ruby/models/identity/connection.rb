  ...
  class Connection
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `XeroRuby::Identity::Connection` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `XeroRuby::Identity::Connection`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'auth_event_id')
        self.auth_event_id = attributes[:'auth_event_id']
      end
    ...
    def hash
      [id, tenant_id, auth_event_id, tenant_type, tenant_name, created_date_utc, updated_date_utc].hash
    end
    ...
    def _deserialize(type, value)
      ...
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        ...
        XeroRuby::Identity.const_get(type).build_from_hash(value)
      end
    end
